<?php $this->display('header.tpl', array('sub_title' => $post['title'])) ?>

<?php if ($error) echo '<div class="error">'. $error .'</div>' ?>

<?php $this->display('title.tpl', array('text' => $post['title'])) ?>

<div class="menu">
	Автор: <a href="<?php echo URL .'profile/'. $post['username'] ?>"><?php echo $post['username'] ?></a><br />
	Время добавления: <?php echo date('d.m.Y в H:i', $post['time']) ?><br />
	<img src="<?php echo URL ?>modules/comments/images/comment.png" alt="" /> <a href="<?php echo a_url('comments', 'module=blog&amp;item_id='. $post['id'] .'&amp;return='. urlencode(a_url('blog/read_more', 'post_id='. $post['id']))) ?>">Комментарии</a> (<?php echo $post['comments'] ?>)
</div>
