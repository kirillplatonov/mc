...
<div class="menu">
    ...
    <hr/>
    <?php echo $post['rating_stars'] ?> (<?php echo $post['rating_voices'] ?> гол.)
    <?php if ( ! $post['rated']): echo '<form method="post" action="'. a_url('blog/rating_change', 'post_id='. $post['id']) .'">
    ...
    </select><input type="submit" value="Ok" /></form>' ?><?php else: ?><br /><?php endif ?>
    <img src="<?php echo URL ?>modules/comments/images/comment.png" alt="" />
    <a href="<?php echo a_url('comments', 'module=blog&amp;item_id='. $post['id'] .'&amp;return='. urlencode(a_url('blog/my'))) ?>">Комментарии</a> (<?php echo $post['comments'] ?>)<br />
