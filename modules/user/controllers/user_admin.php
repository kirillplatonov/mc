...
class User_Admin_Controller extends Controller
{
    ...
    public function action_delete()
    {
        ...
        if (ACCESS_LEVEL <= $user_delete_access)
    ...
    public function action_user_email()
    {
        ...
        if ($_POST['submit']) {
            if (empty($_POST['title']))
                $this->error .= 'Не введен заголовок<br />';
            if (empty($_POST['msg']))
                $this->error .= 'Не введено сообщение заголовок<br />';
    ...
    public function action_ban()
    {
        ...
        if (ACCESS_LEVEL <= $user_ban_access)
            a_error("У вас нет прав на выполнение этой операции!");
    ...
    public function action_edit()
    {
        ...
        if (ACCESS_LEVEL <= $user_edit_access)
            a_error("У вас нет прав на выполнение этой операции!");
    ...
    public function action_list_users()
    {
        ...
        if (!empty($_GET['user_id']))
            ...
            if (!empty($_GET['username']))
                $sql .= " AND username LIKE '%" . a_safe($_GET['username']) . "%'";
            if (!empty($_GET['status']))
                $sql .= " AND status = '" . a_safe($_GET['status']) . "'";
        }

        if (str_safe($_GET['type']) == 'online')
    ...
    public function action_ip_ban()
    {
        ...
        switch (str_safe($_GET['action'])) {
            case 'add':
                ...
                if (isset($_GET['guest_ip'])) {
                    ...
                    if (empty($_GET['guest_ip']))
                        $this->error .= 'Не указан IP адрес, который нужно забанить<br />';

                    // Проверка IP регулярками
                    if (!empty($_GET['guest_ip']) AND ! main::check_input($_GET['guest_ip'], 'IP'))
                        $this->error .= 'Неверно указан IP адрес, формат правильного: ' . main::check_input('', 'IP', 'format') . '<br />';

                    if ($ip = $this->db->get_row("SELECT * FROM #__ip_ban WHERE ip = '" . str_safe($_GET['guest_ip']) . "'"))
                        a_notice('Указаный IP адрес уже забанен!', a_url('user/admin/ip_ban'));
            ...
            case 'edit':
                ...
                if (isset($_GET['id'])) {
                    ...
                    if (!$edit_ip = $this->db->get_row("SELECT * FROM #__ip_ban WHERE id = '" . str_safe($_GET['id']) . "'"))
                        a_error('Не найдена запись с указаным ID!<br />');

                    if (isset($_POST['guest_ip'])) {
                        if (empty($_POST['guest_ip']))
                            $this->error .= 'Не указан IP адрес, который нужно забанить<br />';

                        // Проверка IP регулярками
                        if (!empty($_POST['guest_ip']) AND ! main::check_input($_POST['guest_ip'], 'IP'))
                            $this->error .= 'Неверно указан IP адрес, формат правильного: ' . main::check_input('', 'IP', 'format') . '<br />';

                        if ($ip_edit = $this->db->get_row("SELECT * FROM #__ip_ban WHERE ip = '" . str_safe($_POST['guest_ip']) . "' AND id != $edit_ip[id]"))
                            a_notice('Указаный IP адрес уже забанен!', a_url('user/admin/ip_ban'));
            ...
            case 'delete':
                ...
                if (empty($_GET['guest_ip']))
                    $this->error .= 'Не указан IP адрес, который нужно разбанить<br />';

                // Проверка IP регулярками
                if (!empty($_GET['guest_ip']) AND ! main::check_input($_GET['guest_ip'], 'IP'))
                    $this->error .= 'Неверно указан IP адрес, формат правильного: ' . main::check_input('', 'IP', 'format') . '<br />';

                if (!$ip_delete = $this->db->get_row("SELECT * FROM #__ip_ban WHERE ip = '" . str_safe($_GET['guest_ip']) . "'"))
                    a_error('Не найдена запись с указаным IP адресом!');
        ...
        if (!empty($_GET['guest_ip']))
            $sql .= " AND ip LIKE '%" . a_safe($_GET['guest_ip']) . "%'";
    ...
    public function action_moderate()
    {
        ...
        if ($_GET['user_id'] && $_GET['action']) {
            ...
            # Получаем инфо о пользователе
            if (!$user_moderate = $this->db->get_row("SELECT * FROM #__users WHERE user_id = '" . intval($_GET['user_id']) . "'"))
                a_error("Пользователь не найден!");
